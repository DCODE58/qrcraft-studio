...
-- Create function for creating password-protected QR codes
...
RETURNS TABLE(qr_id UUID, expires_at TIMESTAMP WITH TIME ZONE)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Create function for verifying QR code passwords
CREATE OR REPLACE FUNCTION verify_qr_password(
  qr_id_param UUID,
  password_text TEXT
)
RETURNS TABLE(success BOOLEAN, content_url TEXT, error_message TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
